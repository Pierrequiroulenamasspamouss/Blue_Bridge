...
object RetrofitBuilder {
    ...
    private fun createOkHttpClient(context: Context, isDev: Boolean = false): OkHttpClient {
    ...
    fun getServerApi(context: Context): ServerApi {
        if (serverApi == null || devServerApi == null) {
            ...
            serverApi = createRetrofit(prodUrl, createOkHttpClient(context)).create(ServerApi::class.java)
            devServerApi = createRetrofit(devUrl, createOkHttpClient(context, true)).create(ServerApi::class.java)
        }
...
class FallbackServerApi(
    ...
    override suspend fun getNearbyUsers(latitude: Double, longitude: Double, radius: Double, email: String, token: String) = 
        withFallback { it.getNearbyUsers(latitude, longitude, radius, email, token) }
    ...
    override suspend fun getWeather(latitude: Double, longitude: Double, email: String, token: String) = 
        withFallback { it.getWeather(latitude, longitude, email, token) }
}
