...
class WellViewModel(private val wellDataStore: WellDataStore) : ViewModel() {
    ...
    fun handleConfigEvent(event: WellConfigEvents) {
        _wellData.value = when (event) {
            ...
            is WellConfigEvents.SaveWell -> {
                ...
                persistWellData(event.wellId, dataToSave, wellDataStore, viewModelScope, _errorMessage, _lastSavedData, _wellData)
    ...
    fun loadWellData(wellId: Int) {
        ...
        loadWellData(wellId, wellDataStore, viewModelScope, _wellData, _lastSavedData, _wellLoaded)
    }

    fun resetWellDataState() {
        """ reset all the wells saved """
        resetWellDataState(_wellData, _lastSavedData, _wellLoaded)
    }

    fun restorePreviousWellData() {
        """ restore the local well data to the last saved values """
        restorePreviousWellData(_wellData, _lastSavedData)
    }
    ...
    fun removeWellByIndex(index: Int) {
        ...
        removeWellByIndex(index, wellDataStore, viewModelScope)
    }
    ...
    suspend fun checkDuplicateIpAddress(ip: String, currentWellId: Int): Boolean {
        ...
        return checkDuplicateIpAddress(ip, currentWellId, wellDataStore)
    }
    ...
    fun clearAllWellData() {
        clearAllWellData(wellDataStore, viewModelScope)
    }
